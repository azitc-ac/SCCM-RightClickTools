...
| | | The grids show what a state means, not what the class calls it: `InstallState` as **On the device**, `ResolvedState` as **Deployment** (required / optional / not targeted - it says what the deployment wants, not what is installed), `EvaluationState` as **Status** in words. Each of those cells keeps the raw value in its tooltip. |
