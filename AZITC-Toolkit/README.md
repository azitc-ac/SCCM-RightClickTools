...
| `AZITC-TK-Report.xml`, `AZITC-TK-OpenReport.ps1` | console | Right-click on the Device Collections node or any folder below it: **AZITC: Application compliance report** opens the reporting point's report `Anwendungs-Installationsstatus - Compliance-Übersicht` (sccm-reports repository) in the browser. The script reads the reporting point from `SMS_SCI_SysResUse` through the AdminService and builds the portal URL; report and folder are installer parameters (`-Report`, `-ReportFolder`; `-Report ''` installs no report action). |
